...
**Status: this is no longer a proposal — it is live in `frontend/src/index.css` via a Tailwind v4 `@theme` block.** For the practical "what do I actually write" reference — the full token table, the typography confinement rules, the component library (`Badge`/`Button`/`StatTile`/`RailCard`/`IconButton`), the dark-mockup exception pattern, and a current file-by-file migration status — see **`docs/STYLE_GUIDE.md`**, the actively-maintained companion to this section. What follows here is the condensed version; treat `STYLE_GUIDE.md` as more current if the two ever disagree. Full engineering detail (mechanism, bugs found/fixed): `ENGINEERING_HANDOFF.md` §35.3.
...
**Typography**: Lexend carries every in-product headline and all body text (switched from Inter — chosen for its reading-research-backed legibility, matching this product's grade 4–6 reading-level bar). Anton is confined to `Logo.tsx`'s wordmark only — nowhere else, sitewide (Marketing, Auth/onboarding, and both consoles all lost Anton across two passes). See `docs/STYLE_GUIDE.md` §2 for the exact heading-size convention (`text-2xl font-bold` for a page h1, etc.) every migrated screen follows, and its notes on hero/card-scale display statements that size up from that base by context.
